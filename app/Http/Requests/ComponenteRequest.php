...
class ComponenteRequest extends FormRequest {
    ...
    public function rules() {
        ...
        return [
            ...
            'sim'           => 'nullable|array',
            'sim.id'        => 'exclude_if:sim,null|required_without:sim.serial|exists:TT_Sim,id|unique:TT_Componente,idSim,' . $this->route('id') .  '|unique:TC_ServizioComponente,idSim,' . $this->route('id') . ',idComponente',
            'sim.serial'    => 'exclude_if:sim,null|required_without:sim.id|unique:TT_Sim,serial',
            'sim.idModello' => [
                'required_with:sim.serial',
                Rule::exists('TT_Modello', 'id')->where(function ($query) {
                    $query->where('idTipologia', 11);
                }),
            ],
            'nota'         => 'nullable|array',
            'nota.*.testo' => 'required_with:nota|string'
        ];
    }
}
